import { contractInfo }  from 'evmole';
import { equal, deepEqual } from 'node:assert';

const code = '';

const info = contractInfo(code, { selectors: false, arguments: true, stateMutability: true, disassemble: true });
equal(info.functions.length, 1);
equal(info.functions[0].selector, 'fae7ab82');
equal(info.functions[0].arguments, 'uint32');
equal(info.functions[0].stateMutability, 'pure');
equal(info.storage, undefined);
deepEqual(info.disassembled[1], [2, 'PUSH1 40']);
